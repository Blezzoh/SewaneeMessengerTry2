...
public class Email {
    ...
    // TODO: 8/12/16 make Email instantiated via factory method so that i can choose how initialize the class

    /**
     * only use for first initialization of the table
     * this method will not check for duplication
     *
     * @param inbox needed for access to the user's messages
     ...
     */
    private Email(FullMessage fm) throws IOException, SQLException {
        this(fm.getM());
    }
    ...
    private void initFields(FullMessage fm) {
        ...
        body = fm.getBodyBase64();
        messageId = fm.getId();
        EmailDate emailDate = new EmailDate(fm.getDate());
        date = emailDate.getDefault();
    }

    public Email(Message m) throws IOException, SQLException {
